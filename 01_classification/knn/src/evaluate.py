"""Evaluate the saved K-Nearest Neighbors classifier."""
...
def save_classification_report(
    ...
    """Save detailed class-level metrics."""
...
def save_predictions(
    ...
    """Save predictions, probabilities, and correctness."""
    ...
    results["malignant_probability"] = 1 - probabilities
    results["benign_probability"] = probabilities
...
def save_neighbor_information(
    model,
    x_test: pd.DataFrame,
) -> None:
    """Save distances and indices for one example test observation."""
    ...
    neighbor_table = pd.DataFrame(
        {
            ...
            "training_index": indices[0],
            "distance": distances[0],
        }
    )

    neighbor_table.to_csv(
        PREDICTIONS_DIR / "example_neighbors.csv",
        index=False,
    )
...
def main() -> None:
    """Run the complete KNN evaluation workflow."""
    ...
    save_classification_report(y_test, predictions)
    save_confusion_matrix_values(y_test, predictions)
